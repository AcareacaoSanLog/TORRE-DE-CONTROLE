(function () {
  ...
  function setCloudStatus(message, type = '') {
    els.cloudStatus.textContent = clean(message);
  ...
  function repairVisibleText(root = document.body, force = false) {
    ...
    root.querySelectorAll('[title], [aria-label], [placeholder]').forEach(element => {
      ['title', 'aria-label', 'placeholder'].forEach(attr => {
        ...
        const fixed = repairText(current);
  ...
  function friendlyCloudError(error) {
    const raw = clean(error?.message || error?.details || error?.hint || String(error || 'erro desconhecido'));
    if (!raw) return 'erro desconhecido';
    if (raw.includes('row-level security') || raw.includes('42501')) return 'permissão RLS bloqueou o salvamento';
    if (raw.includes('JWT') || raw.includes('expired')) return 'sessão expirada, saia e entre novamente';
    if (raw.includes('payload') || raw.includes('too large') || raw.includes('413')) return 'arquivo muito grande para salvar na nuvem';
    if (raw.includes('does not exist')) return 'tabela ou coluna não encontrada no Supabase';
  ...
  async function saveCloudState() {
    ...
    try {
      const { error } = await supabaseClient
        .from(CLOUD_TABLE)
        .upsert(
          { id: CLOUD_ID, payload, updated_at: payload.savedAt },
          { onConflict: 'id' }
        )
